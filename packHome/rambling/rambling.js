...
Page({
  ...
  // 更新显示内容
  updateContent: function (number) {
    const index = number - 1;
    if (index >= 0 && index < this.data.mamiLIST.length) {
      // 提取内容部分（去除前面的编号）
      const item = this.data.mamiLIST[index];
  ...
  handleTouchEnd: function () {
    ...
    this.setData({
      isPressing: false,
      currentNumber: this.data.tempNumber
    });

    // 更新显示内容
    this.updateContent(this.data.tempNumber);
  },
  ...
  // 开始随机变化数字
  startRandomChange: function () {
    // 先清除已存在的定时器
    this.stopChanging();

    // 设置新的定时器
    const timerId = setInterval(() => {
      // 生成1到totalCount之间的随机数
      const randomNum = Math.floor(Math.random() * this.data.totalCount) + 1;
      this.setData({
        tempNumber: randomNum,
        // 同时更新currentNumber以便在界面上显示
        currentNumber: randomNum
      });

      // 同时更新显示内容
      this.updateContent(randomNum);
    }, this.data.changeSpeed);
